...
void drawRailwayNetwork(Font customFont, Texture2D stationTexture) {
    ...
    DrawTextEx(customFont, "Симуляция движения поездов!", Vector2{280, 20}, 40,
               0, DARKGRAY);
    DrawTextEx(customFont,
               ("Время: " + network.getSimulationTime().toString()).c_str(),
               Vector2{280, 60}, 30, 0, GRAY);
    ...
    for (std::vector<Station>::iterator it = stations.begin();
        ...
        DrawTextEx(customFont, it->name.c_str(), Vector2{x, y + 25}, 20, 0,
                   DARKGRAY);
    }

    for (std::vector<Train>::iterator it = trains.begin(); it != trains.end();
         ++it) {
        ...
        DrawTextEx(customFont, ("T" + std::to_string(it->id)).c_str(),
                   Vector2{x - 8, ty - 8}, 15, 0, DARKGRAY);
    }
}
...
void drawScheduleWindow(Font customFont, float &RouteForSchedule) {
    ...
    if (r) {
        DrawTextEx(customFont, ("Маршрут: " + r->name).c_str(),
                   Vector2{810, 650}, 20, 0, DARKGRAY);

        float yPos = 680;
        std::vector<StationStop> stops = r->stops;

        for (std::vector<StationStop>::iterator it = stops.begin();
             it != stops.end(); ++it) {
            Station *s = network.getStation(it->stationId);
            if (s) {
                std::string text = s->name + " " + it->arrivalTime.toString() +
                                   "-" + it->departureTime.toString();
                DrawTextEx(customFont, text.c_str(), Vector2{810, yPos}, 20, 0,
                           GRAY);
                yPos += 30;
            }
        }
    ...
    if (routeCount > 0) {
        GuiSlider(routeSlider, "0", std::to_string(routeCount - 1).c_str(),
                  &RouteForSchedule, 0, (routeCount - 1));
    }
}
...
void drawStatisticsWindow(Font customFont) {
    ...
    DrawTextEx(customFont,
               ("События: " + std::to_string(stats.totalEvents)).c_str(),
               Vector2{310, yPos}, 20, 0, GRAY);
    yPos += 30;

    DrawTextEx(customFont,
               ("Задержки: " + std::to_string(stats.totalDelays)).c_str(),
               Vector2{310, yPos}, 20, 0, ORANGE);
    yPos += 30;

    DrawTextEx(
        customFont,
        ("Происшествия: " + std::to_string(stats.totalAccidents)).c_str(),
        Vector2{310, yPos}, 20, 0, RED);
    yPos += 30;

    DrawTextEx(
        customFont,
        ("Время задержки: " + std::to_string(stats.totalDelayMinutes) + " min")
            .c_str(),
        Vector2{310, yPos}, 20, 0, GRAY);
}
...
void drawControlPanel(Font customFont) {
    ...
    DrawTextEx(customFont,
               ("Время: " + network.getSimulationTime().toString()).c_str(),
               Vector2{10, 50}, 20, 0, GRAY);
    DrawTextEx(customFont, ("Поезда: " + std::to_string(trains.size())).c_str(),
               Vector2{10, 75}, 20, 0, GRAY);
    DrawTextEx(customFont, ("Завершено: " + std::to_string(completed)).c_str(),
               Vector2{10, 100}, 20, 0, GREEN);
...
void drawAddTrainDialog(Font customFont) {
    ...
    if (dialogState == 0) {
        DrawTextEx(customFont, "Создание нового маршрута!", Vector2{530, 120},
                   20, 0, DARKGRAY);
        ...
        DrawTextEx(customFont, newRouteName.c_str(), Vector2{525, 195}, 20, 0,
                   DARKGRAY);

        DrawTextEx(customFont, "Начальная станция: ", Vector2{520, 260}, 20, 0,
                   GRAY);
        ...
        DrawTextEx(customFont, "Конечная станция: ", Vector2{520, 330}, 20, 0,
                   GRAY);
        ...
        DrawTextEx(customFont, "Скорость будет вычислена автоматически",
                   Vector2{520, 400}, 20, 0, GRAY);
        ...
        DrawTextEx(customFont, "Выберите промежуточные станции",
                   Vector2{520, 120}, 18, 0, DARKGRAY);
        DrawTextEx(customFont, "(Начальная и конечная уже выбраны)",
                   Vector2{520, 145}, 12, 0, Color{128, 128, 128, 255});
        ...
        for (std::vector<Station>::iterator it = stations.begin();
            ...
            bool isSelected =
                std::find(selectedStops.begin(), selectedStops.end(), it->id) !=
                selectedStops.end();

            std::string label = it->name + (isSelected ? " ✓" : "");
            Color bgColor = isSelected ? Color{100, 200, 100, 255}
                                       : Color{220, 220, 220, 255};

            DrawRectangle(520, yPos, 250, 30, bgColor);
            DrawRectangleLines(520, yPos, 250, 30, DARKGRAY);
            DrawTextEx(customFont, label.c_str(), Vector2{525, yPos + 8}, 12, 0,
                       DARKGRAY);

            if (GuiButton({780, yPos, 30, 30}, "")) {
                std::vector<int>::iterator findIt = std::find(
                    selectedStops.begin(), selectedStops.end(), it->id);
            ...
            DrawTextEx(customFont, isSelected ? "-" : "+",
                       Vector2{790, yPos + 8}, 16, 0, DARKGRAY);
        ...
        if (GuiButton({520, 560, 140, 40}, "")) {
            if (selectedStops.size() >= 2) {
                ...
                Route *newRoute =
                    network.getRoute(network.getRoutes().size() - 1);
                ...
                if (newRoute) {
                    ...
                    for (size_t i = 0; i < selectedStops.size(); ++i) {
                        ...
                        newRoute->addStop(
                            StationStop(selectedStops[i], currentTime,
                                        departureTime, dwellTime));

                        currentTime = departureTime + Time(0, 90);
                    }

                    // Добавляем поезд со скоростью -1 для автоматического
                    // расчёта
...
int main() {
    ...
    Font customFont =
        LoadRussianFontStatic("static/AdwaitaSans-Regular.ttf", 30);
    ...
    while (!WindowShouldClose()) {
        ...
        DrawTextEx(customFont, isRunning ? "СТОП" : "СТАРТ", Vector2{65, 310},
                   20, 0, DARKGRAY);
        ...
        DrawTextEx(customFont, "Добавить маршрут", Vector2{28, 360}, 20, 0,
                   DARKGRAY);
        ...
        DrawTextEx(customFont, "Перезапустить", Vector2{33, 410}, 20, 0,
                   DARKGRAY);
